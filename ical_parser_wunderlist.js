...
function ical_parser(feed_url, callback){
	...
	makeDate = function(ical_date){
		...
		dtWunderlist.date = new Date(dtWunderlist.year, (dtWunderlist.month-1), dtWunderlist.day);
		//Get the full name of the given day
		dtWunderlist.dayname =["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag"][dtWunderlist.date.getDay()];
	...
	this.parseICAL = function(data){
		...
		for(var i=0;i<cal_array.length;i++){
			...
			if(in_event){
				...
				//If the type is a start value date, proccess it and store details
				if(type =='DTSTART;VALUE=DATE:'){
					dtWunderlist = this.makeDate(val);
					val = dtWunderlist.date;
					//These are helpful for display
					//cur_event.start_time = dt.hour+':'+dt.minute;
					cur_event.date = dtWunderlist.day+'/'+dtWunderlist.month+'/'+dtWunderlist.year;
					cur_event.day = dtWunderlist.dayname;
				}
				//Convert timestamp
	...
	this.getFutureEvents = function(){
	   ...
	   return yyyy + (mm[1]?mm:"0"+mm[0]) + (dd[1]?dd:"0"+dd[0]); // padding
		...
		this.events.forEach(function(itm){
			...
			//console.log(itm)
			if(itm['DTSTART;VALUE=DATE'] >= current_date.yyyymmdd()) {
				cur_event = itm

				dtWunderlist = makeDate(itm['DTSTART;VALUE=DATE']);
				val = dtWunderlist.date;
				//These are helpful for display
				//cur_event.start_time = dt.hour+':'+dt.minute;
				cur_event.date = dtWunderlist.day+'.'+dtWunderlist.month+'.'+dtWunderlist.year;
